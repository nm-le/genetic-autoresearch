...
def select_primary(frontier: List[Dict[str, str]], state: Dict[str, Any]) -> Dict[str, str]:
    # Quality-first: while the best elite (lowest val_bpb) is not yet
    # over-explored, prefer it as primary. This counteracts UCB's
    # tendency to spread expansions across known-bad elites because
    # they are "underexplored". The cap is intentionally modest so
    # that after a few unproductive children off the best, the search
    # spreads out via UCB.
    sorted_by_val = sorted(frontier, key=lambda r: float(r.get("val_bpb", 1e9)))
    best_row = sorted_by_val[0]
    best_expansions = int(float(best_row.get("expansions") or 0))
    if best_expansions < 3:
        return best_row
...
def _conceptual_ancestors(commit: str, results: List[Dict[str, str]]) -> set:
    """BFS-walk both parent1 AND parent2 chains (using results.tsv) to
    collect all conceptual ancestors of a commit. Returns a set including
    the commit itself. Following parent2 in addition to parent1 is
    conservative: parent2 in crossover doesn't structurally inherit
    train.py the way parent1 does, but if a mutation idea has been
    *applied* to the lineage via any prior crossover, re-applying it is
    degenerate and we want to skip it."""
    if not commit or commit in {"-", ""}:
        return set()
    parents_map = {}
    for r in results:
        c = r.get("commit", "")
        if c:
            parents_map[c] = (r.get("parent1", ""), r.get("parent2", ""))
    ancestors = set()
    queue = [commit]
    while queue:
        cur = queue.pop()
        if not cur or cur in {"-", ""} or cur in ancestors:
            continue
        ancestors.add(cur)
        p1, p2 = parents_map.get(cur, ("", ""))
        queue.append(p1)
        queue.append(p2)
    return ancestors


def _lineage_independent(a: Dict[str, str], b: Dict[str, str], results: Optional[List[Dict[str, str]]] = None) -> bool:
    """Two elites are lineage-independent iff neither's commit appears
    in the other's full conceptual ancestor chain (walked via parent1
    in results.tsv). This catches both direct and multi-hop ancestral
    relationships — crossover between an elite and one of its conceptual
    ancestors is degenerate because the ancestor's mutations are already
    in the descendant by inheritance."""
    a_commit = a.get("commit", "")
    b_commit = b.get("commit", "")
    if not a_commit or not b_commit:
        return True
    if results is None:
        # Fallback: direct parent/child only (preserves old behavior
        # for callers that haven't been updated)
        a_parent1 = a.get("parent1", "")
        b_parent1 = b.get("parent1", "")
        if a_commit == b_parent1 or b_commit == a_parent1:
            return False
        return True
    a_anc = _conceptual_ancestors(a_commit, results)
    b_anc = _conceptual_ancestors(b_commit, results)
    if b_commit in a_anc or a_commit in b_anc:
        return False
    return True


def choose_operator(frontier: List[Dict[str, str]], state: Dict[str, Any], results: Optional[List[Dict[str, str]]] = None) -> str:
    if len(frontier) < 2:
        return "mutate"

    # Need at least 2 non-baseline elites for crossover to be non-degenerate:
    # baseline elites carry no "idea" to transplant, and choose_secondary
    # already filters them out. Without this guard, choose_secondary's
    # fallback would pick a baseline elite anyway, producing a degenerate
    # crossover (e.g. primary=mutated child, secondary=its own baseline parent).
    non_baseline = [r for r in frontier if r.get("mutation_kind") != "baseline"]
    if len(non_baseline) < 2:
        return "mutate"

    # Need at least one lineage-independent pair: two non-baseline elites
    # where neither appears in the other's conceptual ancestor chain.
    # Without this, a frontier built up via a single mutation chain
    # (elite/0 -> elite/1 -> elite/2) produces degenerate crossovers
    # where the "donor" carries no idea the "base" doesn't already have.
    # Also require at least one such pair to be UNTRIED (not in
    # pair_history). Otherwise we re-run the same crossover when
    # pair_penalty is too weak to flip the choice given no alternatives.
    pair_history = state.get("pair_history", {})
    has_untried_independent_pair = False
    for i, a in enumerate(non_baseline):
        for b in non_baseline[i + 1:]:
            if not _lineage_independent(a, b, results):
                continue
            pair_key = "|".join(sorted([a.get("commit", ""), b.get("commit", "")]))
            if pair_history.get(pair_key, 0) == 0:
                has_untried_independent_pair = True
                break
        if has_untried_independent_pair:
            break
    if not has_untried_independent_pair:
        return "mutate"
...
def choose_secondary(primary: Dict[str, str], frontier: List[Dict[str, str]], state: Dict[str, Any], results: Optional[List[Dict[str, str]]] = None) -> Dict[str, str]:
    pair_history = state.get("pair_history", {})
    best_score = None
    best_row = None
    ...
    for row in frontier:
        ...
        # Skip elites in primary's conceptual mutation lineage (any
        # ancestor or descendant via parent1 chain). The donor's only
        # added "idea" relative to the base is something the base already
        # has (or already lacks), so the crossover is informationally
        # empty.
        if not _lineage_independent(primary, row, results):
            continue
...
def cmd_suggest(args: argparse.Namespace) -> int:
    ...
    operator = choose_operator(frontier, state, results)
    # Per-primary check: if the chosen primary has no untried
    # lineage-independent partner, downgrade to mutate. This catches the
    # case where global crossover is feasible (some other primary has an
    # untried partner) but the picked primary itself has only tried or
    # ineligible partners — avoiding a no-op duplicate crossover.
    if operator == "crossover":
        pair_history = state.get("pair_history", {})
        primary_has_untried_partner = False
        for row in frontier:
            if row.get("tag") == primary.get("tag"):
                continue
            if row.get("mutation_kind") == "baseline":
            ...
            pair_key = "|".join(sorted([primary.get("commit", ""), row.get("commit", "")]))
            if pair_history.get(pair_key, 0) == 0:
                primary_has_untried_partner = True
                break
        if not primary_has_untried_partner:
            operator = "mutate"
    if operator == "mutate":
        suggestion = Suggestion(
            operator="mutate",
        ...
        secondary = choose_secondary(primary, frontier, state, results)
        suggestion = Suggestion(
            operator="crossover",
            parent1=primary["commit"],
            parent1_tag=primary["tag"],
